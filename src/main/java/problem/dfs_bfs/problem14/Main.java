package problem.dfs_bfs.problem14;
...
public class Main {
    ...
    private static Point[] selectedStores;
    ...
    public void dfs(int level, int start) {
        ...
        if (level == m) {
            ...
            for (Point house : houses) {
                ...
                for (Point selectedStore : selectedStores) {
                    distance = Math.min(distance, Math.abs(house.x - selectedStore.x) + Math.abs(house.y - selectedStore.y));
                }
                sum += distance;
            }
            ...
            for (int i = start; i < stores.size(); i++) {
                selectedStores[level] = stores.get(i);
                dfs(level + 1, i + 1);
            }
        }
    }
